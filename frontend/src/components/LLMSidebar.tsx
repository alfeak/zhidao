        ...
        {activeTab === 'remarks' && (
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="p-3 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 shrink-0 transition-colors duration-300">
              <span className="text-[10px] font-bold text-gray-500 dark:text-slate-400 font-mono">
                高亮备注 ({remarks.length})
              </span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {remarks.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-slate-500 text-center py-20">
                  <Compass className="w-8 h-8 stroke-1 text-gray-300 dark:text-slate-700 mb-2" />
                ...
                remarks.map((rem) => (
                  <div
                    key={rem.id}
                    className="bg-white dark:bg-slate-900 p-3 rounded border border-gray-200 dark:border-slate-800 shadow-xs flex flex-col gap-1.5 relative group"
                    style={{ borderLeftWidth: '3px', borderLeftColor: rem.color }}
                  >
                    <div className="flex items-center justify-between text-[10px] text-gray-400 dark:text-slate-500 font-mono">
                      <span>段落: #{rem.blockId.split('_').pop()}</span>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-slate-950/10">
            ...
            <div className="flex items-center justify-between border-b border-gray-200 dark:border-slate-800 pb-2.5">
              ...
              >
                ...
                <span>{showKeys ? '隐藏密钥' : '显示密钥'}</span>
              </button>
            </div>

            {/* OpenAI-compatible models */}
            <div className="space-y-3">
              <div className="flex items-center gap-1.5 border-b border-gray-100 dark:border-slate-800 pb-1.5">
                <Activity className="w-3.5 h-3.5 text-black dark:text-white" />
                <span className="text-xs font-bold text-gray-800 dark:text-slate-200">已配置模型列表</span>
              </div>

              {/* Models list */}
              <div className="space-y-2.5">
                {(!config.models || config.models.length === 0) ? (
                  <div className="p-3 bg-white dark:bg-slate-900 rounded border border-dashed border-gray-300 dark:border-slate-800 text-center text-[10px] text-gray-400">
                    暂未配置模型
                  </div>
                ) : (
                  config.models.map((mod) => (
                    ...
                    >
                      ...
                      <div className="mt-2 space-y-2 text-[10px]">
                        <div>
                          <span className="text-gray-400 dark:text-slate-450">Base URL: </span>
                        ...
                        <div className="space-y-1">
                          <span className="text-gray-400 dark:text-slate-450 block">API Key:</span>
                        ...
                        <div className="pt-1">
                          <button
                            ...
                            className="w-full py-1 border border-gray-200 dark:border-slate-700 hover:border-black dark:hover:border-slate-300 rounded text-[9px] font-bold text-gray-600 dark:text-slate-300 hover:text-black dark:hover:text-white transition-all cursor-pointer flex items-center justify-center gap-1"
                          >
                            {testingId === mod.id ? (
                              <>
                          ...
                          {testResults[mod.id] && (
                            <div className={`mt-1.5 p-2 rounded text-[9px] leading-relaxed flex items-start gap-1 ${
              ...
              <form onSubmit={handleAddNewModel} className="bg-white dark:bg-slate-900 p-3.5 rounded-lg border border-gray-200 dark:border-slate-800 shadow-xs space-y-2.5 transition-colors duration-300">
                <div className="border-b border-gray-100 dark:border-slate-800 pb-1.5 flex items-center gap-1">
                  ...
                  <span className="text-xs font-bold text-gray-800 dark:text-slate-200">添加模型</span>
                </div>

                <div className="space-y-2.5">
                  {/* Model Name */}
                  <div className="space-y-1">
                    <span className="text-[10px] font-medium text-gray-500 dark:text-slate-400 block">模型名称 <span className="text-red-500">*</span></span>
                    <input
                      type="text"
                  ...
                  {/* API Key */}
                  <div className="space-y-1">
                    <span className="text-[10px] font-medium text-gray-500 dark:text-slate-400 block">API 密钥 <span className="text-red-500">*</span></span>
                    <div className="relative">
                      <input
                        type={showKeys ? 'text' : 'password'}
                  ...
                  {/* Base URL */}
                  <div className="space-y-1">
                    <span className="text-[10px] font-medium text-gray-500 dark:text-slate-400 block">Base URL (可选)</span>
                    <div className="relative">
                      <input
                        type="text"
                  ...
                  {/* Is Primary Checkbox */}
                  <label className="flex items-center gap-2 cursor-pointer pt-0.5 select-none text-gray-650 dark:text-slate-350">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-[10px] font-medium">设为首选模型</span>
                  </label>

                  {/* Temporary testing result */}
                  {testResults['new_model_temp'] && (
                    <div className={`p-2 rounded text-[9px] leading-relaxed flex items-start gap-1 ${
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      ...
                      className="flex-1 py-1.5 border border-gray-200 dark:border-slate-700 hover:border-black dark:hover:border-slate-300 rounded text-[10px] text-gray-700 dark:text-slate-200 hover:text-black dark:hover:text-white font-bold transition-all cursor-pointer flex items-center justify-center gap-1"
                    >
                    ...
                    <button
                      type="submit"
                      className="flex-1 py-1.5 bg-black dark:bg-slate-100 dark:text-slate-900 hover:bg-gray-800 dark:hover:bg-slate-200 text-white rounded text-[10px] font-bold transition-all cursor-pointer flex items-center justify-center gap-1"
                    >
