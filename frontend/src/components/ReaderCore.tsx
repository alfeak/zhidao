...
const MarkdownBlockCard = memo(function MarkdownBlockCard({ block, paperId, remarks, selected, onSelect, onAddRemark, onDeleteRemark, onContextMenu }: MarkdownBlockCardProps) {
  ...
  >
    ...
    <div className="absolute right-2 bottom-1.5 z-10 flex gap-1 rounded border border-slate-200 bg-white/90 px-1.5 py-0.5 text-[11px] text-slate-600 opacity-0 shadow-xs transition-opacity group-hover:opacity-100 dark:border-slate-700 dark:bg-slate-800/90 dark:text-slate-300"><button type="button" onClick={(event) => { event.stopPropagation(); setIsRemarkEditorOpen((open) => !open); }} className="flex items-center gap-1 font-medium hover:text-cyan-600 dark:hover:text-cyan-400 cursor-pointer"><PenTool className="h-2.5 w-2.5" /><span>Remark</span></button></div>
